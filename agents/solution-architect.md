...
- Hand the chosen approach to `planner` to produce a phased implementation plan with file ownership, dependency order, and a test matrix. Review the plan; push back if a phase has no failure-mode analysis or no measurable "done".
...
- **Exit criteria:** all plan phases implemented; the build runs; deviations from the plan are documented.
...
2. If the user gives feedback on one gate ("latency still high", "verify again"), re-enter at that gate — do not restart from scratch.
3. If the user provides a new challenge, archive the prior `_workspace/` and start fresh.
4. Preserve earlier success criteria unless the user changes them; re-verify against the full set.
...
5. Own Gate 4 yourself — run the final verification and write the verdict; do not delegate the sign-off.
